...
class Base extends Model
{
   ...
   * @return [type]
   */
  public static function getList($condition, $relevance = false, $orders = false, $is_array = false, $limit = false, $or_where = false)
  {
    try
    {
...
// dd(DB::getQueryLog()); //获取查询语句、参数和执行时间
   ...
   * @return 分页数据
   */
  public static function getPaging($condition, $relevance = false, $orders = false, $is_array = false, $pageSize = 10, $field = false, $groups= false, $or_where = false)
  {
    try
    {
      $model = self::buildWhere($condition, $relevance);
